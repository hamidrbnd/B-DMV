...
#To determine percentage of agents in majority
r_num=int(input("Please enter percentage of agents in majority: "))
...
if(range_ctrl == 1): #For different number of agents
    ...
    num_maj = int(((r_num)/100)*number_agent)
...
Opr0=np.mean(topr0)/number_agent
Msg0=np.mean(tmsg0)

Opr1=np.mean(topr1)/number_agent
Msg1=np.mean(tmsg1)

Opr2=np.mean(topr2)/number_agent
Msg2=np.mean(tmsg2)
