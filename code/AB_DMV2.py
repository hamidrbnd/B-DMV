...
def AB_DMV2(msgcnt, init,adj_matrix,v,mem):
    tmp_choice=list()
    tmp_mem=list()
    ...
    for i in range (len(neighbor)):
        ...
        tmp_mem = tmp_mem + list(mem[neighbor[i]][1])
    #for these algorithm    
    ...
    if(cntrl_mem == 0):
        tmp_rpt = [tmp_mem.count(list(set(tmp_mem))[i]) for i in range(len(list(set(tmp_mem))))]
        if(len(tmp_rpt) != 0):
            if(tmp_rpt.count(max(tmp_rpt)) == 1):
                cntrl_mem = 1
                mem_update = {list(set(tmp_mem))[tmp_rpt.index(max(tmp_rpt))]}      
    #end    
